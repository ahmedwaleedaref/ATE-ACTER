...
| `min_n` / `max_n` | candidate length bounds in tokens (default 2–4; `data_layout.md` §5.4 notes published best models top out at 4-token English terms — provisional pending `Data_stats.md` §7 item 3, term-length distribution, not yet measured) |
...
- `min_n`/`max_n = 2–4` is a placeholder pending `Data_stats.md` §7 item 3
  (term length distribution). Revisit once that number lands.
...
accident. Standard next step: `spaCy` (or similar) with an `Adj*Noun+`-style
chunk pattern in place of the boundary heuristic, restricting candidates to
actual noun phrases. This is the dependency change flagged as a "revisit
trigger" in §1.2 — still needs an explicit ask-and-approve before adding it
to `requirements.txt`, per `CLAUDE.md`, not a silent swap.
